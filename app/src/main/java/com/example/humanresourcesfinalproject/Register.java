...
public class Register extends AppCompatActivity {
    ...
    private void createAdminsTable() {
        ...
        // Create a dummy admin object
        Admin dummyAdmin = new Admin(
                "admin1", "Admin", "User", "123456789", "admin@gmail.com", "Admin123",
                "M", "001", "MySchool", "John Doe", "987654321", "12", "Health Fund", "None", "Vegetarian", true, true, true
        );

        // Use Firebase Authentication to create a user (dummy admin)
        mAuth.createUserWithEmailAndPassword(dummyAdmin.getEmail(), dummyAdmin.getPassword())
                .addOnCompleteListener(this, task -> {
                    if (task.isSuccessful()) {
                        // If registration is successful, get the UID generated by Firebase Authentication
                        FirebaseUser firebaseUser = mAuth.getCurrentUser();

                        if (firebaseUser != null) {
                            String adminId = firebaseUser.getUid(); // Get the UID from Firebase Authentication

                            // Save the dummy admin under the generated UID
                            adminsRef.child(adminId).setValue(dummyAdmin).addOnCompleteListener(adminTask -> {
                                if (adminTask.isSuccessful()) {
                                    Log.d("AdminTable", "Admin created successfully with UID: " + adminId);
                                } else {
                                    Log.e("AdminTable", "Failed to save admin data", adminTask.getException());
                                }
                            });
                        }
                    } else {
                        Log.e("AdminTable", "Failed to create dummy admin user", task.getException());
                    }
                });
    }
